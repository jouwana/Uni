...
float defaultStepTranslate = 0.5f;
...
{
	return vec2(renderer->getWidth() / 2, renderer->getHeight() / 2);
...
void keyboard( unsigned char key, int x, int y )
{
	...
	/*if ((Zdeg < 0.044 && Zdeg > 0) || (Zdeg > -0.044 && Zdeg < 0))
		Zdeg = 0;
	if ((Xdeg < 0.044 && Xdeg > 0) || (Xdeg > -0.044 && Xdeg < 0))
		Xdeg = 0;
	if ((Ydeg < 0.044 && Ydeg > 0) || (Ydeg > -0.044 && Ydeg < 0))
		Ydeg = 0;*/
	/*cout << "xdeg: " << Xdeg << "  ydeg: " << Ydeg << "    zdeg: " << Zdeg << endl;*/
...
void mouse(int button, int state, int x, int y)
{
	...
	{
		bool flag = scene->activateOnClick(x, renderer->getHeight() - y);
		if (flag)
		{
			worldTrans = false;
			scene->worldTransforming = false;
...
void motion(int x, int y)
{
	...
	if (flag)
	{
		if (!firstClick)
		{
			...
			scene->worldTransforming = false;
			dy = -dy;
			scene->transformActiveModel(Translate(dx, dy, 1), Translate(-dx, -dy, 1));
...
void shadeMenu(int id)
{
	...
	{
	case SHADING_FLAT:
		renderer->set_shading_alg(FLAT);
		glutPostRedisplay();
		break;
	case SHADING_GOURAND:
		renderer->set_shading_alg(GOURAND);
		glutPostRedisplay();
		break;
	case SHADING_PHONG:
		renderer->set_shading_alg(PHONG);
		glutPostRedisplay();
		break;
	}
}
