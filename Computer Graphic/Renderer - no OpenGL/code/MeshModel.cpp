...
{	//if this is the first time we render, initialize array
	...
	*/
	...
	for (int i = 0; i < face_counter * 3; i++)
	{
		...
		z = z * percent; // scale p using the same percentage.
...
void MeshModel::loadFile(string fileName)
{
	...
	while (!ifile.eof())
	{
		...
		{
			textures.push_back(vec3fFromStream(issLine));
		}
	...
	texture_positions = new vec3[face_counter * 3];
	vec3* avgNormals = new vec3[vertices.size()];
	for (int i = 0; i < vertices.size(); i++)
		avgNormals[0] = vec3(0,0,0);
	...
	for (vector<FaceIdcs>::iterator it = faces.begin(); it != faces.end(); ++it)
	{
		for (int i = 0; i < 3; i++)
		{
			...
			if (it->vn[i] != 0)
			{
				avgNormals[it->v[i] - 1] += (normals[it->vn[i] - 1]);
			}
			if (it->vt[i] != 0)
	...
	for (vector<FaceIdcs>::iterator it = faces.begin(); it != faces.end(); ++it)
	{
		for (int i = 0; i < 3; i++)
		{
			...
			if (it->vn[i] != 0)
			{
				normal_positions[k] = normalize(avgNormals[it->v[i] - 1]);
				norm_count++;
			}
...
bool MeshModel::clickedOn(int x, int y, mat4& trans, mat4& proj)
{
	if (firstDraw)
		return false;
	setScaledMinAndMax();
